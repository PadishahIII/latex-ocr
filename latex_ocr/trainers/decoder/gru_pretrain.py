...
from latex_ocr.trainers.decoder.gru_pretrain_model import (
    GRUDecoder,
)
...
logger = get_logger_by_name("latex_ocr_decoder_pretrain")


def get_gru_decoder_model(cfg: ModelCfg) -> GRUDecoder:
...
cfg = TrainerCfg(
    ...
    data_cfg=data_cfg,
    model_factory=get_gru_decoder_model,
    device=get_device(),
    ...
    MLflow=config.MLflowCfg(
        experiment_name="latex-ocr-gru-decoder",
        mlflow_model_name="latex-ocr-gru-decoder",
        dataset_name="both",
        desc="",
    ...
    Checkpoint=config.CheckpointCfg(
        ...
        # resume_strict=True,
        # mlflow_run_id="3b6a6ad6f92c4b8b91b9ba966023c809",
    )

)


class DecoderPretrainTrainer(AdaptiveTrainableRecipe):
    ...
    def _loss_step(self, batch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        labels = batch["labels"]
        logits = self._model(None, labels)

        # Model returns (B, T-1, V) predicting tokens 1..T-1.
...
@cli.command()
def train() -> None:
    recipe = DecoderPretrainTrainer(cfg=cfg, is_tune_mode=False)
...
def smoke_cpu() -> None:
    ...
    local_cfg.Train.enable_amp = True

    # Smaller model for quick CPU check.
    local_cfg.model_cfg.vocab_size = 128
    local_cfg.model_cfg.gru_decoder_cfg = GRUDecoderCfg(
        emb_dim=32,
        dec_dim=32,
        attn_dim=32,
        freeze_encoder=True,
    ...
    model = get_gru_decoder_model(local_cfg.model_cfg).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3)
    loss_fn = CrossEntropyLoss(ignore_index=0)

    torch.manual_seed(0)
    labels = torch.randint(0, local_cfg.model_cfg.vocab_size, (2, 16), dtype=torch.long)
    labels[:, 0] = local_cfg.model_cfg.bos_id

    model.train()
    logits = model(None, labels)
...
if __name__ == "__main__":
    """
    CPU smoke test:
      PYTHONPATH=. uv run python latex_ocr/trainers/decoder/gru_pretrain.py smoke_cpu

    Full pretraining (downloads HF dataset; still CPU if your cfg.device is cpu):
      PYTHONPATH=. uv run python latex_ocr/trainers/decoder/gru_pretrain.py train
      PYTHONPATH=. nohup uv run python latex_ocr/trainers/decoder/gru_pretrain.py train 2>&1 &> pretrain.log &
    """
